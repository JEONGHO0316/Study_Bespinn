...
#앱 서비스 
az webapp create -g rg-paaslab -p asp-linux -n appgbhjh --runtime "NODE:22-lts"
...
az webapp deployment source config-zip --resource-group rg-paaslab --src api.zip --name apihjh

#4. api 앱 동작 테스트

#5. web 앱 코드 배포
az webapp deployment source config-zip --resource-group rg-paaslab --src web.zip --name webhjh
