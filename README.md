# study_bespin
## PowerShell (GUI & CLI)

- Searching Command : [Shell Practice](codes/10_powershell.sh)
- File & directory Command : [Shell Practice](codes/20_controll_file_dir_powershells.sh)

## Other 
- Other : [Othere List](codes/Othere/)

## Linux 
- Linux : [Linux Practice List](codes/Linux_Practice/)

## Network ( Linux )
- Network ( Linux ) : [Linux Network Practice List](codes/Linux_Network/)

## AWS 

- AWS : [AWS List](codes/AWS/)

## DB
- DB : [DBA List](codes/DBA/)

## Azure 
- Azure : [AZURE List](codes/AZURE/)

## GCP 
- GCP : [GCP List](codes/GCP/)

## K8S 
- Kubernetes : [K8S List](codes/K8S)

## Terraform 
- Terraform : [Terraform List](codes/Terraform/)



